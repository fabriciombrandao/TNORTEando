...
_step_check_system() {
  ...
  # Internet — testa múltiplos hosts (VPS pode bloquear google.com)
  _internet_ok=false
  for _host in github.com cloudflare.com 1.1.1.1 8.8.8.8; do
    if curl -fsSL --max-time 5 "https://${_host}" > /dev/null 2>&1 ||        ping -c1 -W3 "${_host}" > /dev/null 2>&1; then
      _internet_ok=true
      break
    fi
  done
