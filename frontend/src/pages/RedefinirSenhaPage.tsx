...
export default function RedefinirSenhaPage() {
  ...
  async function redefinir() {
    ...
    try {
      await api.post("/api/v1/auth/redefinir-senha", { token, senha });
      setSucesso(true);
      ...
      <div className="w-full max-w-sm animate-fade-in">
        ...
        {sucesso ? (
          <div className="card p-6 text-center">
            <p className="text-emerald-600 font-semibold">Senha redefinida com sucesso!</p>
          ...
          <div className="card p-6 shadow-xl shadow-slate-100 space-y-4">
            ...
            <button onClick={redefinir} disabled={!valida || loading} className="btn-primary btn-lg btn-full">
              {loading ? "Redefinindo..." : "Redefinir senha"}
            </button>
            <button onClick={() => navigate("/login")} className="btn-secondary btn-md btn-full">
              Voltar ao login
            </button>
          </div>
        )}
